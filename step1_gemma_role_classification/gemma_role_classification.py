...
# Script for classifying job role combinations into SASB categories using Ollama
# Sends rows in batches to the LLM (one API call → JSON array response)
...
parser = argparse.ArgumentParser(
    description="Classify job role combinations into SASB categories with Ollama"
)
parser.add_argument("--shard-id", type=int, default=0)
parser.add_argument("--num-shards", type=int, default=6)
parser.add_argument(
    "--input-csv", type=str,
    default="/gpfs/home/fl488/process_RL_sasb/role_combinations.csv",
)
parser.add_argument(
    "--output-csv", type=str,
    default="/gpfs/home/fl488/process_RL_sasb/output_sasb.csv",
)
parser.add_argument(
    "--max-concurrent", type=int, default=10,
    help="Maximum number of concurrent batch requests to Ollama",
)
parser.add_argument(
    "--rows-per-call", type=int, default=5,
    help="Number of role rows to send per LLM API call",
)
...
# Longer timeout for batch calls (more tokens in/out than single-row sentiment)
CLIENT_TIMEOUT = ClientTimeout(total=600)

# Load the prompt template from the same directory as this script
_HERE = os.path.dirname(os.path.abspath(__file__))
PROMPT_TEMPLATE_PATH = os.path.join(_HERE, "gemma_role_classification_prompt.md")
...
class RoleBatch:
    batch_id: int
    positions: List[int]       # integer row positions in the full DataFrame
    rows: List[Dict[str, str]] # each: {"role_k1500_v3": ..., "role_k5000_v3": ..., "role_k10000_v3": ...}
...
def build_prompt(row: Dict[str, str]) -> str:
    """Fill the prompt template for a single role row.

    Combines the three granularity levels into a natural label:
    e.g. "Software Engineer - Backend Engineer - Database Management Specialist"
    """
    parts = [row["role_k1500_v3"], row["role_k5000_v3"], row["role_k10000_v3"]]
    # Deduplicate consecutive identical levels (e.g. ".NET Developer - .NET Developer - Azure Developer")
    label_parts = [parts[0]]
    for p in parts[1:]:
        if p != label_parts[-1]:
            label_parts.append(p)
    role_label = " - ".join(label_parts)
    return PROMPT_TEMPLATE.replace("{role_label}", role_label)
...
def clean_result(result: Any) -> Optional[Dict]:
    ...
    if confidence not in {"high", "medium", "low"}:
        confidence = "low"
    return {"sasb_categories": valid_cats, "confidence": confidence}


async def call_ollama_api(
    session: aiohttp.ClientSession,
    batch: RoleBatch,
) -> Tuple[RoleBatch, str, str]:
    """Send a single role row to Ollama and return the raw response."""
    # Each batch contains exactly one row
...
async def call_ollama_api_with_retry(
    ...
    max_retries: int = 5,
    initial_delay: int = 5,
) -> Tuple[RoleBatch, Dict]:
    """Call Ollama with retries. Returns the batch paired with a single cleaned result dict."""

    # These may appear directly or embedded inside a 500 response body.
    # Must be checked BEFORE the generic 500 handler so they trigger a proper restart.
    ...
    for attempt in range(1, max_retries + 1):
        ...
        # Runner crash signal — short wait, Ollama will restart the runner automatically
        ...
        # Connection / EOF errors — check BEFORE the generic 500 handler because Ollama
        # often wraps these in a 500 body (e.g. "POST predict: EOF", "connection reset by peer").
        ...
        # Generic HTTP 5xx with no connection error in body — short backoff
        ...
        # Clean result (invalid SASB names are dropped, not retried)
...
async def main_async():
    print("✅ SASB classification main started", flush=True)
    ...
    required_cols = {"role_k1500_v3", "role_k5000_v3", "role_k10000_v3"}
    if not required_cols.issubset(df.columns):
        raise ValueError(f"Input CSV must contain columns: {required_cols}")

    # Normalise: fill NaN with empty string
    ...
    # Split work across shards by row position
    ...
    # Build one RoleBatch per row (single-row processing)
    batches: List[RoleBatch] = []
    for pos in shard_positions:
        row = {
            "role_k1500_v3": df.iloc[pos]["role_k1500_v3"],
            "role_k5000_v3": df.iloc[pos]["role_k5000_v3"],
            "role_k10000_v3": df.iloc[pos]["role_k10000_v3"],
        }
        batches.append(RoleBatch(batch_id=len(batches), positions=[pos], rows=[row]))

    print(f"Created {len(batches)} single-row jobs", flush=True)
