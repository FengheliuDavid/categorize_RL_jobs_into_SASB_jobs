You are an ESG expert. Classify the job posting below into SASB categories based on the PRIMARY responsibilities described.
...
- A category qualifies ONLY if the job posting explicitly states responsibilities that directly manage that SASB area as a core function
- Use both the role labels and the full description text — explicit keywords and duties are the only basis for classification
- Most postings map to 0–2 categories; when in doubt, return `[]` — a false negative is better than a false positive
- Generic duties (e.g. "ensure compliance", "work in a team", "report to management") do NOT qualify unless the specific SASB topic is named

**The key test:** Does the posting explicitly describe managing, overseeing, or being accountable for that SASB area? Merely working at a company that operates in a relevant industry is not sufficient.

**Common false positives to avoid:**
- Generic compliance or legal language → NOT Business_Ethics unless fraud/bribery/corruption is explicitly named
- "Customer data" or "databases" → NOT Customer_Privacy or Data_Security unless privacy/security governance is explicit
- "Health benefits" or "safe work environment" → NOT Employee_Health_&_Safety unless safety management is a core duty
- "Diverse team" or "inclusive culture" → NOT Employee_Engagement,_Diversity_&_Inclusion unless D&I programs are a primary responsibility
- "Supply chain" or "procurement" → NOT Supply_Chain_Management unless ESG supplier risk is explicitly mentioned
- Any generic administrative, support, or back-office duties → 0 categories

## Output

Return ONLY a JSON object, no explanation:
{"sasb_categories": ["Category_Name"]}

If no categories apply: {"sasb_categories": []}

## Job Posting

**Role labels:** {role_k1500_v3} / {role_k5000_v3} / {role_k10000_v3}
