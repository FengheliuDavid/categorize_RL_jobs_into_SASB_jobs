...
# Script for classifying job postings (description + role labels) into SASB categories using Ollama
...
parser = argparse.ArgumentParser(
    description="Classify job postings (description + role labels) into SASB categories with Ollama"
)
...
parser.add_argument(
    "--output-csv", type=str,
    default="/gpfs/home/fl488/process_RL_sasb/gemma_combined_classification/gemma_combined_classification_output.csv",
)
parser.add_argument(
...
PROMPT_TEMPLATE_PATH = os.path.join(_HERE, "gemma_combined_classification_prompt.md")
...
class DescriptionBatch:
    batch_id: int
    positions: List[int]
    rows: List[Dict[str, str]]  # each: {"position_id": ..., "description": ..., "role_k1500_v3": ..., "role_k5000_v3": ..., "role_k10000_v3": ...}
...
def build_prompt(row: Dict[str, str]) -> str:
    ...
    return (
        PROMPT_TEMPLATE
        .replace("{job_description}", row.get("description", "").strip())
        .replace("{role_k1500_v3}", row.get("role_k1500_v3", "").strip())
        .replace("{role_k5000_v3}", row.get("role_k5000_v3", "").strip())
        .replace("{role_k10000_v3}", row.get("role_k10000_v3", "").strip())
    )
...
async def main_async():
    print("✅ SASB combined classification main started", flush=True)
    ...
    required_cols = {"position_id", "description", "role_k1500_v3", "role_k5000_v3", "role_k10000_v3"}
    if not required_cols.issubset(df.columns):
        raise ValueError(f"Input CSV must contain columns: {required_cols}")

    df["description"] = df["description"].fillna("").astype(str)
    df["role_k1500_v3"] = df["role_k1500_v3"].fillna("").astype(str)
    df["role_k5000_v3"] = df["role_k5000_v3"].fillna("").astype(str)
    df["role_k10000_v3"] = df["role_k10000_v3"].fillna("").astype(str)
    ...
    for pos in shard_positions:
        ...
        role1 = r["role_k1500_v3"].strip()
        role2 = r["role_k5000_v3"].strip()
        role3 = r["role_k10000_v3"].strip()
        if not desc and not role1 and not role2 and not role3:
        ...
        row = {
            ...
            "role_k1500_v3": role1,
            "role_k5000_v3": role2,
            "role_k10000_v3": role3,
        }
        batches.append(DescriptionBatch(batch_id=len(batches), positions=[pos], rows=[row]))

    print(f"Created {len(batches)} single-row jobs ({len(skipped)} skipped — no description or roles)", flush=True)
